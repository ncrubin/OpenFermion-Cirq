...
from openfermioncirq.primitives.optimal_givens_decomposition import front_and_back_givens_decomposition
...
def test_circuit_generation_and_accuracy():
    for dim in range(2, 10):
        ...
        assert numpy.allclose(true_unitary.conj().T @ true_unitary, numpy.eye(2 ** dim, dtype=complex))

        test_unitary = cirq.unitary(circuit)
        assert numpy.isclose(abs(numpy.trace(true_unitary.conj().T @ test_unitary)), 2**dim)
...
def test_circuit_generation_state():
    ...
    circuit.append([cirq.X(qubits[0]), cirq.X(qubits[1]), cirq.X(qubits[1]),
                    cirq.X(qubits[2]), cirq.X(qubits[3]), cirq.X(qubits[3])])   # alpha-spins are first then beta spins
    ...
    test_final_state = test_unitary @ wavefunction
