...
"""
A routine for constructing a circuit to exactly implement a unitary generated by one-body rotations through
the optimal Givens rotation network.  Construction of this circuit can be found in
Optica Vol. 3, Issue 12, pp. 1460-1465 (2016).
"""
import numpy
import cirq
...
def front_and_back_givens_decomposition(qubits: Sequence[cirq.QubitId],
    ...
    Args:
        qubits: Sequence of qubits to apply the operations over.  The qubits should be ordered in linear
                physical order.
        unitary:
    """
    N = unitary.shape[0]
    ...
    for i in range(1, N):
        if i % 2 == 1:
            for j in range(0, i):
                ...
                gmat = givens_matrix_elements(unitary[N - j - 1, i - j - 1], unitary[N - j - 1, i - j - 1 + 1],
                                              which='left')
            ...
            for j in range(1, i + 1):
                ...
                gmat = givens_matrix_elements(unitary[N + j - i - 1 - 1, j - 1], unitary[N + j - i - 1, j - 1],
                                              which='right')
    ...
    for (left_gmat, (i, j)) in reversed(left_rotations):
        ...
        if not numpy.allclose(new_phase_matrix @ new_givens_matrix.conj(), matrix_to_decompose):
    ...
    for (gmat, (i, j)) in list(reversed(new_left_rotations)) + list(
        ...
        if not numpy.isclose(gmat[0, 0].imag, 0.0):
            raise GivensMatrixError(
                "Givens matrix does not obey our convention that all elements in the first column are real")
